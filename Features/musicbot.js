...
module.exports = async function (bot, msg, commands, prefix){
    ...
    if ((msg.channel.name == "zene")) {
        ...
        // Stop
        if (msg.content.startsWith(prefix + commands[19])) {
            const UserVoiceID = msg.member.voiceChannel.id
            let bot1NotInThis = false
            let bot2NotInThis = false
        ...
        // Skip
        if (msg.content.startsWith(prefix + commands[20])) {
        ...
        // Back
        if (msg.content.startsWith(prefix + commands[21])) {
        ...
        // Resume
        if (msg.content.startsWith(prefix + commands[22])) { 
        ...
        // Pause
        if (msg.content.startsWith(prefix + commands[23])) { 
            if(!msg.member.voiceChannel) return ErrorMsg("Pause Parancs", "Nem vagy benne egy szobában sem!", msg.author.id)
            let botWhoIn = null
            msg.member.voiceChannel.members.map(m => {
                if(m.id === bots[0]){
        ...
        // Shuffle
        if (msg.content.startsWith(prefix + commands[24])) {
        ...
        // Repeat
        if (msg.content.startsWith(prefix + commands[25])) {
        ...
        // NoRepeat
        if (msg.content.startsWith(prefix + commands[26])) {
        ...
        // Queue
        if (msg.content.startsWith(prefix + commands[27])) { 
        ...
        // Add
        if (msg.content.startsWith(prefix + commands[28])) { 
        ...
        // Remove
        if (msg.content.startsWith(prefix + commands[29])) {
